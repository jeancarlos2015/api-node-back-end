...
function insertContato(product, res, db){
    var name = product.name;
    var description = product.description;
    var price = product.price;
    var currency = product.currency;

    var sql = `insert into Contato (nome, telefone, data, codoperadora,cor) 
            VALUES 
            (?, ?, ?, ?);`;
...
function validateRequest(req, res) {
    var fs = require('fs');
    var schema = JSON.parse(fs.readFileSync('api/mock/ContatosMock.json', 'utf8'));
